...
1. [인프라 구성](#1-인프라-구성)
2. [Druid 아키텍처 — 컴포넌트 역할 및 데이터 흐름](#2-druid-아키텍처--컴포넌트-역할-및-데이터-흐름)
3. [전체 Kustomize 구조](#3-전체-kustomize-구조)
4. [사용법 (Quick Start)](#4-사용법-quick-start)
5. [각 파일 설명](#5-각-파일-설명)
6. [스토리지 컴포넌트 선택](#6-스토리지-컴포넌트-선택)
7. [주요 설계 결정 및 이유](#7-주요-설계-결정-및-이유)
8. [알려진 이슈](#8-알려진-이슈) (8-5: Jetty 12 Router 스레드 부족 포함)
9. [트러블슈팅 가이드](#9-트러블슈팅-가이드)
10. [Kafka 구성 참고](#10-kafka-구성-참고)
...
> Indexer를 2개로 늘리면 HA 구성이 됩니다. 방법과 이유는 [7-3. Indexer HA 구성](#7-3-indexer-ha-구성-replicas2)을 참고하세요.

---


## 2. Druid 아키텍처 — 컴포넌트 역할 및 데이터 흐름
...
### 2-5. MiddleManager vs Indexer — 수집 흐름 비교
...
└── overlays/
    ...
    │       └── statefulsets-with-aws-region.yaml      # AWS_REGION 주입 (대안, 기본 주석 처리)
...
| `AWS_REGION` | **config patch 또는 StatefulSet patch** | AWS SDK가 S3 엔드포인트 리전 결정에 사용 | S3/MinIO 연결 실패 가능 |
...
S3/MinIO를 사용하지 않는 환경(local emptyDir, Azure, GCS 등)에는 리전 설정이 불필요합니다.  
S3/MinIO를 사용할 때 AWS SDK v2는 명시적 리전이 필수입니다. 두 가지 방법으로 제공할 수 있습니다:

**방법 1 (권장): ConfigMap patch의 `druid.s3.endpoint.signingRegion`**

```properties
# overlays/<your-cluster>/patches/druid-common-config-with-minio.yaml  (또는 -with-aws-s3.yaml)
druid.s3.endpoint.signingRegion=us-east-1   # MinIO: 임의 값 / AWS S3: 버킷 리전
```

이것은 Druid-native 설정으로, S3 client builder에 직접 리전을 설정합니다. Druid의 S3 client에만 적용되며 StatefulSet 변경이 필요 없습니다.

**소스 근거** — 메인 configuration reference 페이지에는 없지만 S3 extension 문서 및 소스 코드에서 확인:

| 항목 | 내용 |
|------|------|
| 공식 문서 | [S3 extension — Endpoint config 테이블](https://druid.apache.org/docs/37.0.0/development/extensions-core/s3) |
| 소스 파일 | `extensions-core/s3-extensions/src/main/java/org/apache/druid/storage/s3/S3StorageDruidModule.java` |
| 메서드 | `getServerSideEncryptingAmazonS3Builder()` |
| 바인딩 클래스 | `AWSEndpointConfig` (property prefix: `druid.s3.endpoint`) |
| SDK 호출 | `Region.of(endpointConfig.getSigningRegion())` → S3 client builder에 전달 |

관련 소스 코드 발췌 (Druid 37.0.0):
```java
final Region region = StringUtils.isNotEmpty(endpointConfig.getSigningRegion())
    ? Region.of(endpointConfig.getSigningRegion())
    : null;
// region → S3ClientBuilder에 전달 → AWS SDK v2 리전 요구사항 충족
```

**방법 2 (대안): StatefulSet patch를 통한 `AWS_REGION` env var**
...
Pod의 OS 환경변수 레벨에서 리전을 설정하므로 같은 Pod 내 모든 AWS SDK 컴포넌트에 영향을 줍니다. Pod 내 다른 AWS SDK 컴포넌트도 리전이 필요한 경우에만 사용하세요.

이 overlay의 스토리지 ConfigMap patch(`druid-common-config-with-minio.yaml`, `druid-common-config-with-aws-s3.yaml`)에는 이미 `druid.s3.endpoint.signingRegion`이 포함되어 있으므로 `statefulsets-with-aws-region.yaml`은 기본적으로 주석 처리되어 있습니다.
...
spec:
  volumeClaimTemplates:
    - metadata:
        name: data
      spec:
        accessModes: ["ReadWriteOnce"]   # 반드시 명시 — SMP가 spec을 merge가 아닌 replace로 처리
        storageClassName: longhorn       # 클러스터 기본값 재정의 (기본값 사용 시 생략)
        resources:
          requests:
            storage: 50Gi              # 크기 재정의
```

> **주의**: `accessModes`는 `pvc-patch.yaml`에 반드시 포함해야 합니다.  
> Kustomize SMP는 `volumeClaimTemplates` 각 항목의 `spec` 하위 객체를 field-by-field 병합이 아닌 replace로 처리합니다.  
> `accessModes`를 생략하면 `components/pvc`에서 설정한 값이 사라지고 Kubernetes가 다음 오류로 거부합니다:  
...
patches:
  ...
  # AWS_REGION env (대안) — 기본적으로 주석 처리됨
  # 리전은 위 ConfigMap patch의 druid.s3.endpoint.signingRegion으로 이미 설정됩니다.
  # OS env 레벨에서 AWS_REGION이 필요한 경우에만 활성화하세요.
...
druid.s3.endpoint.signingRegion=us-east-1   # 임의 값; MinIO는 리전 무시
...
druid.s3.endpoint.signingRegion=<BUCKET_REGION>  # 예: ap-northeast-2
```
...
### 7-2. Per-Task 로그 파일 (log4j2 RoutingAppender)
...
이를 위해 Kafka의 메모리를 먼저 절감했습니다 (아래 7-4 참고).
...
### 7-5. ZooKeeper 없는 Kubernetes 서비스 발견
...
### 8-1. Task "Could not allocate segment" 실패
...
### 8-2. Kubernetes Watch 만료로 Indexer 오프라인 인식 (해결됨)
...
### 8-3. Overlord 재시작 시 완료된 Task 이력 소멸
...
### 8-5. Druid 37 + Jetty 12 — Router managementProxy 스레드 부족 (해결됨)
...
### 8-6. Control-plane(plane1) 메모리 증가 — apiserver watch 누적
